...
public class MissionService {
    ...
    /**
     * Creates a new mission with initial configuration.
     * 
     * @param mission the mission to create
     * @return the created mission with generated ID
     */
    public Mission createMission(Mission mission) {
        ...
        // Generate mission identifier if not provided
        if (mission.getIdentifier() == null || mission.getIdentifier().trim().isEmpty()) {
            mission.setIdentifier(generateMissionIdentifier(mission));
        }
     ...
     * @param updatedMission the updated mission data
     * @return the updated mission
     * @throws IllegalArgumentException if mission not found
     */
    public Mission updateMission(UUID missionId, Mission updatedMission) {
        Mission existingMission = missionRepository.findById(missionId)
        ...
        existingMission.setDescription(updatedMission.getDescription());
        existingMission.setPriority(updatedMission.getPriority());
        existingMission.setClassificationLevel(updatedMission.getClassificationLevel());
        existingMission.setObjective(updatedMission.getObjective());
        existingMission.setOperationalArea(updatedMission.getOperationalArea());
        existingMission.setExpectedDurationHours(updatedMission.getExpectedDurationHours());
        existingMission.setUpdatedAt(LocalDateTime.now());
        
        return missionRepository.save(existingMission);
    }

    /**
     * Retrieves all missions with pagination support.
     * 
     * @param pageable pagination information
     * @return page of missions
     */
     ...
     * Retrieves missions by status with pagination.
     * 
     * @param status the mission status to filter by
     * @param pageable pagination information
     * @return page of missions with specified status
     */
    @Transactional(readOnly = true)
    public Page<Mission> getMissionsByStatus(MissionStatus status, Pageable pageable) {
        return missionRepository.findByStatus(status, pageable);
    ...
    /**
     * Retrieves missions by priority level.
     * 
     * @param priority the priority level to filter by
     * @return list of missions with specified priority
     */
    @Transactional(readOnly = true)
    public List<Mission> getMissionsByPriority(MissionPriority priority) {
        return missionRepository.findByPriorityOrderByCreatedAtDesc(priority);
    }
     ...
     * @return the updated mission
     */
    public Mission assignUnitToMission(UUID missionId, UUID unitId) {
        Mission mission = missionRepository.findById(missionId)
            .orElseThrow(() -> new IllegalArgumentException("Mission not found: " + missionId));
        ...
        if (!isUnitAvailableForMission(unit)) {
            throw new IllegalStateException("Unit is not available for mission assignment: " + unit.getCallsign());
        }
     ...
     * @return the updated mission
     */
    public Mission updateMissionStatus(UUID missionId, MissionStatus newStatus) {
        Mission mission = missionRepository.findById(missionId)
            .orElseThrow(() -> new IllegalArgumentException("Mission not found: " + missionId));
        ...
        } else if (newStatus == MissionStatus.COMPLETED && mission.getActualEndTime() == null) {
     ...
     * @return the created waypoint
     */
    public MissionWaypoint addWaypointToMission(UUID missionId, MissionWaypoint waypoint) {
        Mission mission = missionRepository.findById(missionId)
            .orElseThrow(() -> new IllegalArgumentException("Mission not found: " + missionId));
        ...
        // Validate coordinates
        if (!GeospatialUtils.isValidCoordinates(waypoint.getLatitude(), waypoint.getLongitude())) {
            throw new IllegalArgumentException("Invalid waypoint coordinates");
        }
    ...
    /**
     * Retrieves all waypoints for a mission ordered by sequence.
     * 
     * @param missionId the mission ID
     * @return list of waypoints
     */
    @Transactional(readOnly = true)
    public List<MissionWaypoint> getMissionWaypoints(UUID missionId) {
        return waypointRepository.findByMissionIdOrderBySequenceNumber(missionId);
    }

    /**
     * Creates a mission status report.
     * 
     * @param missionId the mission ID
     * @param report the report to create
     * @return the created report
     */
    public MissionReport createMissionReport(UUID missionId, MissionReport report) {
        Mission mission = missionRepository.findById(missionId)
            .orElseThrow(() -> new IllegalArgumentException("Mission not found: " + missionId));
    ...
    /**
     * Retrieves all reports for a mission.
     * 
     * @param missionId the mission ID
     * @return list of mission reports
     */
    @Transactional(readOnly = true)
    public List<MissionReport> getMissionReports(UUID missionId) {
        return reportRepository.findByMissionIdOrderByCreatedAtDesc(missionId);
    }

    /**
     * Calculates mission progress as a percentage.
     * 
     * @param missionId the mission ID
     * @return progress percentage (0-100)
     */
    @Transactional(readOnly = true)
    public double calculateMissionProgress(UUID missionId) {
        Mission mission = missionRepository.findById(missionId)
            .orElseThrow(() -> new IllegalArgumentException("Mission not found: " + missionId));
        
        List<MissionWaypoint> waypoints = getMissionWaypoints(missionId);
        
        if (waypoints.isEmpty()) {
            return mission.getStatus() == MissionStatus.COMPLETED ? 100.0 : 0.0;
        }
        
        long completedWaypoints = waypoints.stream()
            .mapToLong(wp -> wp.getArrivalTime() != null ? 1 : 0)
            .sum();
        
        return (double) completedWaypoints / waypoints.size() * 100.0;
    }

    /**
     * Deletes a mission and all associated data.
     * 
    ...
    public void deleteMission(UUID missionId) {
        ...
        // Only allow deletion of missions in PLANNING or CANCELLED status
        if (mission.getStatus() != MissionStatus.PLANNING && mission.getStatus() != MissionStatus.CANCELLED) {
            throw new IllegalStateException("Cannot delete mission in current status: " + mission.getStatus());
        }
    ...
    private void validateMissionData(Mission mission) {
        if (mission.getName() == null || mission.getName().trim().isEmpty()) {
            throw new IllegalArgumentException("Mission name is required");
        }
        
        if (mission.getObjective() == null || mission.getObjective().trim().isEmpty()) {
            throw new IllegalArgumentException("Mission objective is required");
        }
        
        if (mission.getPriority() == null) {
            throw new IllegalArgumentException("Mission priority is required");
        }
        
        if (mission.getExpectedStartTime() == null) {
            throw new IllegalArgumentException("Expected start time is required");
        }
        
        if (mission.getExpectedEndTime() == null) {
            throw new IllegalArgumentException("Expected end time is required");
        }
        
        if (mission.getExpectedStartTime().isAfter(mission.getExpectedEndTime())) {
            throw new IllegalArgumentException("Start time must be before end time");
        }
    }

    private String generateMissionIdentifier(Mission mission) {
        String priorityCode = mission.getPriority().name().substring(0, 1);
        String dateCode = DateTimeUtils.getCurrentTacticalTime().substring(0, 6);
        String randomCode = String.format("%03d", (int) (Math.random() * 1000));
        
        return String.format("MSN-%s%s-%s", priorityCode, dateCode, randomCode);
    }

    private boolean isUnitAvailableForMission(MilitaryUnit unit) {
        // Check if unit is already assigned to an active mission
        List<Mission> activeMissions = missionRepository.findByAssignedUnitAndStatusIn(
            unit, List.of(MissionStatus.PLANNING, MissionStatus.ACTIVE));
        
        return activeMissions.isEmpty() && 
               unit.getStatus().name().equals("ACTIVE") && 
               unit.getReadinessLevel().matches("C[12]");
    }
