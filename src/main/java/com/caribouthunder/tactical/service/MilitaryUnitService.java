...
import com.caribouthunder.tactical.domain.UnitStatusHistory;
import com.caribouthunder.tactical.repository.MilitaryUnitRepository;

/**
 * Service class for Military Unit operations.
 * 
 * Provides business logic for military unit management including CRUD operations,
...
public class MilitaryUnitService {
     ...
     * @throws IllegalArgumentException if callsign already exists
     */
    public MilitaryUnit createUnit(MilitaryUnit unit) {
        if (militaryUnitRepository.existsByCallsign(unit.getCallsign())) {
            throw new IllegalArgumentException("Unit with callsign '" + unit.getCallsign() + "' already exists");
        }
     ...
     * @param updatedUnit the updated unit data
    ...
    public MilitaryUnit updateUnit(Long id, MilitaryUnit updatedUnit) {
        ...
        if (!existingUnit.getCallsign().equals(updatedUnit.getCallsign()) && 
            ...
            throw new IllegalArgumentException("Unit with callsign '" + updatedUnit.getCallsign() + "' already exists");
        }
     ...
     * @param changeReason the reason for status change
    ...
    public MilitaryUnit updateUnitStatus(Long id, MilitaryUnit.UnitStatus newStatus, 
        ...
        if (!previousStatus.equals(newStatus)) {
            ...
            UnitStatusHistory statusHistory = new UnitStatusHistory(unit, previousStatus, newStatus, changedBy);
     ...
     * @return the updated military unit
     */
    public MilitaryUnit updateUnitPosition(Long id, BigDecimal latitude, BigDecimal longitude, BigDecimal altitude) {
        MilitaryUnit unit = militaryUnitRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Military unit not found with id: " + id));
     ...
     *
     * @param id the unit ID
     * @param readinessLevel the new readiness level
     * @return the updated military unit
     */
     ...
     *
     * @param id the unit ID
     * @param threatLevel the new threat level
     * @return the updated military unit
     */
     ...
     *
     * @param callsign the unit callsign
     * @return the military unit
     */
    @Transactional(readOnly = true)
     ...
     *
     * @param domain the operational domain
     ...
     */
    @Transactional(readOnly = true)
    public List<MilitaryUnit> getUnitsWithinRadius(BigDecimal centerLat, BigDecimal centerLon, Double radiusKm) {
     ...
     * 
     * @param readinessLevel readiness level
     ...
     */
    public List<MilitaryUnit> findUnitsWithinRadius(double latitude, double longitude, double radiusKm) {
        return militaryUnitRepository.findUnitsWithinRadius(
